...
func (gs *GameServer) Patch(delta *GameServer) ([]byte, error) {
	...
	if err != nil {
		return result, errs.Wrapf(err, "error marshalling to json current GameServer %s", gs.ObjectMeta.Name)
	}

	newJSON, err := json.Marshal(delta)
	if err != nil {
		return result, errs.Wrapf(err, "error marshalling to json delta GameServer %s", delta.ObjectMeta.Name)
	}

	patch, err := jsonpatch.CreatePatch(oldJSON, newJSON)
	if err != nil {
		return result, errs.Wrapf(err, "error creating patch for GameServer %s", gs.ObjectMeta.Name)
	}
	...
	return result, errs.Wrapf(err, "error creating json for patch for GameServer %s", gs.ObjectMeta.Name)
}
...
func (gs *GameServer) UpdateCount(name string, action string, amount int64) error {
	if action != GameServerPriorityIncrement && action != GameServerPriorityDecrement {
		return errs.Errorf("unable to UpdateCount with Name %s, Action %s, Amount %d. Allocation action must be one of %s or %s", name, action, amount, GameServerPriorityIncrement, GameServerPriorityDecrement)
	}
	if amount < 0 {
		return errs.Errorf("unable to UpdateCount with Name %s, Action %s, Amount %d. Amount must be greater than 0", name, action, amount)
	}
	...
	return errs.Errorf("unable to UpdateCount with Name %s, Action %s, Amount %d. Counter not found in GameServer %s", name, action, amount, gs.ObjectMeta.GetName())
}
...
func (gs *GameServer) UpdateCounterCapacity(name string, capacity int64) error {
	if capacity < 0 {
		return errs.Errorf("unable to UpdateCounterCapacity: Name %s, Capacity %d. Capacity must be greater than or equal to 0", name, capacity)
	}
	if counter, ok := gs.Status.Counters[name]; ok {
	...
	return errs.Errorf("unable to UpdateCounterCapacity: Name %s, Capacity %d. Counter not found in GameServer %s", name, capacity, gs.ObjectMeta.GetName())
}
...
func (gs *GameServer) UpdateListCapacity(name string, capacity int64) error {
	...
	if capacity < 0 || capacity > ListMaxCapacity {
		return errs.Errorf("unable to UpdateListCapacity: Name %s, Capacity %d. Capacity must be between 0 and 1000, inclusive", name, capacity)
	}
	...
	return errs.Errorf("unable to UpdateListCapacity: Name %s, Capacity %d. List not found in GameServer %s", name, capacity, gs.ObjectMeta.GetName())
}
...
func (gs *GameServer) AppendListValues(name string, values []string) error {
	if values == nil {
		return errs.Errorf("unable to AppendListValues: Name %s, Values %s. Values must not be nil", name, values)
	}
	if list, ok := gs.Status.Lists[name]; ok {
	...
	return errs.Errorf("unable to AppendListValues: Name %s, Values %s. List not found in GameServer %s", name, values, gs.ObjectMeta.GetName())
}
...
func (gs *GameServer) DeleteListValues(name string, values []string) error {
	if values == nil {
		return errs.Errorf("unable to DeleteListValues: Name %s, Values %s. Values must not be nil", name, values)
	}
	if list, ok := gs.Status.Lists[name]; ok {
	...
	return errs.Errorf("unable to DeleteListValues: Name %s, Values %s. List not found in GameServer %s", name, values, gs.ObjectMeta.GetName())
}
