...
func (ext *Extensions) creationMutationHandler(review admissionv1.AdmissionReview) (admissionv1.AdmissionReview, error) {
	...
	if err != nil {
		return review, ext.errs.Wrap(err, fmt.Sprintf("error marshalling default applied GameServer %s to json", gs.ObjectMeta.Name))
	}

	patch, err := jsonpatch.CreatePatch(obj.Raw, newGS)
	if err != nil {
		return review, ext.errs.Wrap(err, fmt.Sprintf("error creating patch for GameServer %s", gs.ObjectMeta.Name))
	}

	jsonPatch, err := json.Marshal(patch)
	if err != nil {
		return review, ext.errs.Wrap(err, fmt.Sprintf("error creating json for patch for GameServer %s", gs.ObjectMeta.Name))
	}
...
func (ext *Extensions) creationValidationHandler(review admissionv1.AdmissionReview) (admissionv1.AdmissionReview, error) {
	...
	err := json.Unmarshal(obj.Raw, gs)
	if err != nil {
		return review, ext.errs.Wrap(err, fmt.Sprintf("error unmarshalling GameServer json after schema validation: %s", obj.Raw))
	}
...
func (ext *Extensions) creationMutationHandlerPod(review admissionv1.AdmissionReview) (admissionv1.AdmissionReview, error) {
	...
	if err := json.Unmarshal([]byte(annotation), &passthroughPortAssignmentMap); err != nil {
		return review, ext.errs.Wrap(err, fmt.Sprintf("could not unmarshal annotation %v (value %q)", passthroughPortAssignmentMap, annotation))
	}
	...
	if err != nil {
		return review, ext.errs.Wrap(err, fmt.Sprintf("error marshalling changes applied Pod %s to json", pod.ObjectMeta.Name))
	}

	patch, err := jsonpatch.CreatePatch(obj.Raw, newPod)
	if err != nil {
		return review, ext.errs.Wrap(err, fmt.Sprintf("error creating patch for Pod %s", pod.ObjectMeta.Name))
	}

	jsonPatch, err := json.Marshal(patch)
	if err != nil {
		return review, ext.errs.Wrap(err, fmt.Sprintf("error creating json for patch for Pod %s", pod.ObjectMeta.Name))
	}
...
func (c *Controller) syncGameServerDeletionTimestamp(ctx context.Context, gs *agonesv1.GameServer) (*agonesv1.GameServer, error) {
	...
	if pod != nil && !isDev {
		...
		if pod.ObjectMeta.DeletionTimestamp.IsZero() {
			...
			if err != nil {
				return gs, c.errs.Wrap(err, fmt.Sprintf("error deleting pod for GameServer. Name: %s, Namespace: %s", gs.ObjectMeta.Name, pod.ObjectMeta.Namespace))
			}
	...
	return gs, c.errs.Wrap(err, fmt.Sprintf("error removing finalizer for GameServer %s", gsCopy.ObjectMeta.Name))
}
...
func (c *Controller) syncGameServerPortAllocationState(ctx context.Context, gs *agonesv1.GameServer) (*agonesv1.GameServer, error) {
	...
	if err != nil {
		...
		return gs, c.errs.Wrap(err, fmt.Sprintf("error updating GameServer %s to default values", gs.Name))
	}
...
func (c *Controller) syncGameServerCreatingState(ctx context.Context, gs *agonesv1.GameServer) (*agonesv1.GameServer, error) {
	...
	gs, err = c.gameServerGetter.GameServers(gs.ObjectMeta.Namespace).Update(ctx, gsCopy, metav1.UpdateOptions{})
	if err != nil {
		return gs, c.errs.Wrap(err, fmt.Sprintf("error updating GameServer %s to Starting state", gs.Name))
	}
	return gs, nil
}
...
func (c *Controller) syncDevelopmentGameServer(ctx context.Context, gs *agonesv1.GameServer) (*agonesv1.GameServer, error) {
	...
	gs, err := c.gameServerGetter.GameServers(gs.ObjectMeta.Namespace).Update(ctx, gsCopy, metav1.UpdateOptions{})
	if err != nil {
		return gs, c.errs.Wrap(err, fmt.Sprintf("error updating GameServer %s to %v status", gs.Name, gs.Status))
	}
	return gs, nil
}
...
func (c *Controller) createGameServerPod(ctx context.Context, gs *agonesv1.GameServer) (*agonesv1.GameServer, error) {
	...
	if err != nil {
		...
		default:
			...
			return gs, c.errs.Wrap(err, fmt.Sprintf("error creating Pod for GameServer %s", gs.Name))
		}
	}
...
func (c *Controller) syncGameServerStartingState(ctx context.Context, gs *agonesv1.GameServer) (*agonesv1.GameServer, error) {
	...
	if err != nil {
		return gs, c.errs.Wrap(err, fmt.Sprintf("error retrieving node %s for Pod %s", pod.Spec.NodeName, pod.ObjectMeta.Name))
	}
	gsCopy := gs.DeepCopy()
	...
	gs, err = c.gameServerGetter.GameServers(gs.ObjectMeta.Namespace).Update(ctx, gsCopy, metav1.UpdateOptions{})
	if err != nil {
		return gs, c.errs.Wrap(err, fmt.Sprintf("error updating GameServer %s to Scheduled state", gs.Name))
	}
...
func (c *Controller) syncGameServerRequestReadyState(ctx context.Context, gs *agonesv1.GameServer) (*agonesv1.GameServer, error) {
	...
	gs, err = c.gameServerGetter.GameServers(gs.ObjectMeta.Namespace).Update(ctx, gsCopy, metav1.UpdateOptions{})
	if err != nil {
		return gs, c.errs.Wrap(err, fmt.Sprintf("error setting Ready, Port and address on GameServer %s Status", gs.ObjectMeta.Name))
	}
...
func (c *Controller) syncGameServerPodIPs(ctx context.Context, gs *agonesv1.GameServer) (*agonesv1.GameServer, error) {
	...
	gs, err = c.gameServerGetter.GameServers(gs.ObjectMeta.Namespace).Update(ctx, gsCopy, metav1.UpdateOptions{})
	return gs, c.errs.Wrap(err, fmt.Sprintf("error updating GameServer %s with new PodIPs", gsCopy.ObjectMeta.Name))
}
...
func (c *Controller) applyGameServerReadyContainerIDAnnotation(ctx context.Context, gsCopy *agonesv1.GameServer, pod *corev1.Pod) (*agonesv1.GameServer, error) {
	...
	if _, ok := pod.ObjectMeta.Annotations[agonesv1.GameServerReadyContainerIDAnnotation]; !ok {
		...
		if _, err := c.podGetter.Pods(pod.ObjectMeta.Namespace).Update(ctx, podCopy, metav1.UpdateOptions{}); err != nil {
			return nil, c.errs.Wrap(err, fmt.Sprintf("error updating ready annotation on Pod: %s", pod.ObjectMeta.Name))
		}
	}
	return gsCopy, nil
}
...
func (c *Controller) syncGameServerShutdownState(ctx context.Context, gs *agonesv1.GameServer) error {
	...
	if err != nil {
		return c.errs.Wrap(err, fmt.Sprintf("error deleting Game Server %s", gs.ObjectMeta.Name))
	}
...
func (c *Controller) moveToErrorState(ctx context.Context, gs *agonesv1.GameServer, msg string) (*agonesv1.GameServer, error) {
	...
	gs, err := c.gameServerGetter.GameServers(gs.ObjectMeta.Namespace).Update(ctx, gsCopy, metav1.UpdateOptions{})
	if err != nil {
		return gs, c.errs.Wrap(err, fmt.Sprintf("error moving GameServer %s to Error State", gs.ObjectMeta.Name))
	}
...
func (c *Controller) gameServerPod(gs *agonesv1.GameServer) (*corev1.Pod, error) {
	...
	return pod, c.errs.Wrap(err, fmt.Sprintf("error retrieving pod for GameServer %s", gs.ObjectMeta.Name))
}
